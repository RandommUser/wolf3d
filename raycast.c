...
void	traycast(t_game game)
{
	...
	while (++x < game.mlx.size.x)
	{
		...
		while (!hit && ++len < RDIST)
		{
			...
			hit = is_transparent(game.map.start, map);
			if (!hit && is_wall(game.map.start, map))
				return ;
		}
		if (!hit)
			return ;
	...
	while (++x < game.mlx.size.x)
	{
		...
		while (!hit && ++len < RDIST)
		{
			...
			//if (block)// && (block->block == 1 || block->block == 2 || block->block == 4 || block->block == 3)) // ADD MORE WALL CHECKS
				hit = is_wall(game.map.start, map);
		...
		if (hit && block->block == 7)
		{
			color = 0xffffff;
			draw.x = draw.y - (draw.y - draw.x) / 10;
		}
		color = !hit ? 0x333333 : color;
