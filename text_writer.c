...
static t_print	print_split(t_print print, int *prog, int *i)
{
	...
	if (print.str[*i - 1] == '-')
		*i = *i - 1;
	mlx_string_put(print.mlx_ptr, print.mlx_win, *prog - *i * TEXT_WIDTH,
	...
	{
		*prog += TEXT_WIDTH * 4;
		if (!in_screen(dot(*prog, print.pos.y), print.size, print.str)) // test if wrapping after tab is needed
		{
			*prog = print.pos.x;
			print.pos.y += TEXT_HEIGHT;
		}
...
void	write_to_screen(t_print print)
{
	int		i;
	int		prog;
	...
	prog = print.pos.x;
	while (print.str[++i] && print.pos.y < print.size.y)
	{
		...
		!in_screen(dot(prog, print.pos.y), print.size, &print.str[i + 1]))// encounters any other kind of white-space or - and wrapping is needed
		{
			if (print.str[i] == '-')
			{
				i++;
...
void	write_to_editor(t_editor *edit, t_dot pos, int color, char *str)
{
	...
	print.mlx_ptr = edit->mlx_ptr;
	print.mlx_win = edit->mlx_win;
	print.size = edit->size; // screensize / bottom right corner edge to write in. will overflow the last line in Y axis for the last line within the area
	print.pos = pos;
	print.str = str;
	print.color = color;
